...
<?
	$poulequery = mysql_query ("SELECT * FROM ploegen WHERE toernid='$toernooiid' AND poule='$poule' ORDER BY punten DESC, saldo DESC", $db) or die(mysql_error());
     ?>
     <table><tr><th></th><th>team</th><th>naam</th><th>p</th><th>g</th><th>v</th><th>t</th><th>s</th></tr>
     <?
	     $plaats=1;
     while ($poulequeryrow = mysql_fetch_array($poulequery))
     {
	  echo "<tr><td class=standright><b>".$plaats.".</b></td><td class=standright>".$poulequeryrow['teamnr']."</td><td class=standleft>".$poulequeryrow['naam']."</td><td class=standright>".$poulequeryrow['punten']."</td><td class=standright>".$poulequeryrow['gespeeld']."</td><td class=standright>".$poulequeryrow['voor']."</td><td class=standright>".$poulequeryrow['tegen']."</td><td class=standright>".$poulequeryrow['saldo']."</td></tr>\n";
	  $plaats++;
     }
...
<h2 align=left>Stand</h2>
<?
	$poulequery = mysql_query ("SELECT * FROM ploegen WHERE toernid='$toernooiid' AND poule='$poule2' ORDER BY punten DESC, saldo DESC", $db) or die(mysql_error());
     ?>
     <table><tr><th></th><th>team</th><th>naam</th><th>p</th><th>g</th><th>v</th><th>t</th><th>s</th></tr>
     <?
	     $plaats=1;
